...
public class XMLConfigBuilder {
    ...
    /**
     *
     * メイン配置フイルムを読み取り、ｄｂにつながるのに必要な情報を取って、Configurationオブジェクトにパッケージ
     * する
     *    利用する技術は  dom4j+xpath
     */
    public static Configuration loadConfiguration(InputStream in){
        try{
            //Configurationオブジェクトのインスタンス
            Configuration cfg = new Configuration();

            //1.SAXReaderオブジェクトを作る
            SAXReader reader = new SAXReader();
            //2.configを読み取り、documentオブジェクトを作る
            Document document = reader.read(in);
            //3.XMLの"root<>"をゲット
            Element root = document.getRootElement();
            //4.xpathで “＜property＞”こういう項目をすべて取り出す
            List<Element> propertyElements = root.selectNodes("//property");
            //5.ノードを遍歴する
            for(Element propertyElement : propertyElements){
               //nameプロパティのValueを取りだす、判断する
